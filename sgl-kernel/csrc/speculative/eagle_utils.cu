...
// Ragged version: each request can have different draft_token_num
// token_indptr [bs + 1]: cumulative offsets for tokens (includes verified)
// score_indptr [bs + 1]: cumulative offsets for selected_index (excludes verified)
// mask_indptr [bs + 1]: cumulative offsets for tree_mask
__global__ void build_tree_efficient_ragged(
    int64_t* parent_list,
    int64_t* selected_index,
    int64_t* verified_seq_len,
    bool* tree_mask,
  ...
  if (tid == 0) {
    ...
    for (int i = draft_token_num - 1; i > 0; --i) {
      // retrive_index stores the global flattened index into padded candidates array
      ...
      if (parent_tb_idx > 0) {
        int parent_token_idx = parent_list[bid * (topk * (depth - 1) + 1) + parent_tb_idx];
        for (; parent_position < draft_token_num; ++parent_position) {
          if (parent_position < num_scores &&
              selected_index[score_offset + parent_position] == parent_token_idx) {
            ++parent_position;
            break;
          }
        }
...
void build_tree_kernel_efficient_ragged(
  ...
  dim3 grid(bs);
  dim3 block(max_draft_token_num);
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();

  build_tree_efficient_ragged<<<grid, block, 0, stream>>>(
      ...
      static_cast<int64_t*>(token_indptr.data_ptr()),
      static_cast<int64_t*>(score_indptr.data_ptr()),
      static_cast<int64_t*>(mask_indptr.data_ptr()),
      int32_t(topk),
      int32_t(depth),
